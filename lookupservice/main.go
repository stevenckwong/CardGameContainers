...
func lookupHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if isInstrumentationOn() {
		// this entire section is disabled as I am no longer using this for NewRelic tracing
...
func defaultHandler(writer http.ResponseWriter, request *http.Request) {
	fmt.Fprintf(writer, "<html><head><title>Card Game Lookup Service</title></head>")
	fmt.Fprintf(writer, "<body>")
	fmt.Fprintf(writer, "<h1>Instructions</h1>")
	fmt.Fprintf(writer, "<body>")
	fmt.Fprintf(writer, "<a href='/lookup?gameCommCalc'>Look up Game Commission Calculator URL</a><br/><br/>")
	fmt.Fprintf(writer, "<a href='/getVersion'>Get Version of Lookup Service</a><br/><br/>")
	fmt.Fprintf(writer, "To update the Commission Calculator url, enter the following url:<br/>")
	str := "http://[currenturl]/setService?[newhostname]:[newport]<br/>"
	str += "For e.g. <br/>"
	str += "http://localhost:8889/setService?newhost:8890<br/>"
...
func getGameCommCalcURL() string {
	...
	url := "http://localhost:8090/commCalc"
...
func main() {
	/* === No longer using this to demo NewRelic ===
